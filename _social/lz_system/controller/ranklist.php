<?php
	...
	if(empty($_GET['type'])||$_GET['type']=='time')
		$data = $db->get_all("select i.* from lz_item as i,lz_category as c  where i.category_id = c.category_id AND c.parent_id =1 AND i.status!=0 order by publish_time desc LIMIT 20");
