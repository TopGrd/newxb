...
if ($m == 'new_item')
{
	...
	$data = filter_array($_POST,'DOI,reference,file_name,classnum,name!,name_en,time,html,description!,keywords,intval:category_id!,intval:order_id,author,authoren,institution,institutionen');
...
else if ($m == 'edit_item')
{
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$map = "update lz_author set author='".$_author."',institution='".$_institution."',author_en='".$_authoren."',institution_en='".$_institutionen."' where aid=".$item_id;
		$db->query($map);
	}
